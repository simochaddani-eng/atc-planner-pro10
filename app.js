// app.js - Version avec bouton Réduire le menu fonctionnel (CSS)
...
function setupEvents() {
  ...
  // --- CORRECTION ULTIME DU BOUTON REDUIRE MENU ---
  ...
  if (collapseBtn) {
    collapseBtn.addEventListener('click', function(e) {
      ...
      // Si le menu est ouvert (pas encore réduit)
      if (!sidebar.classList.contains('collapsed')) {
        // On le cache
        sidebar.classList.add('collapsed');
        // On étend le contenu principal
        content.classList.add('expanded');
        // On change le texte du bouton
        this.innerHTML = '›&nbsp; Ouvrir le menu';
      } else {
        // Si le menu est déjà réduit, on le rouvre
        sidebar.classList.remove('collapsed');
        content.classList.remove('expanded');
        this.innerHTML = '‹&nbsp; Réduire le menu';
      }
    });
  }
